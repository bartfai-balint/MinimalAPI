...
app.MapGet("/", () => "Welcome to the Jungle");
...
app.MapPost("/superheroes", async (DataContext context, SuperHero hero) =>
    {
...
app.MapPut("/superheroes/{id}", async (DataContext context, SuperHero hero, int id) =>
{
    var dbHero = await context.SuperHeroes.FindAsync(id);
    if (dbHero == null) return Results.NotFound("Not found");
...
app.MapDelete("/superheroes{id}", async (DataContext context, int id) =>
{
    var dbHero = await context.SuperHeroes.FindAsync(id);
    if (dbHero == null) return Results.NotFound("Not found");
